...
class PlotMPLTask(pyloco.PylocoTask):
    ...
    def __init__(self, parent):
        ...
        self.add_option_argument("--pyplot", metavar="args@funcname", action="append",
                help="(E) pyplot function name and arguments for the function.")
    ...
    def perform(self, targs):
        ...
        for idx in range(self.env['num_pages']):
            ...
            if targs.pyplot:
                for opt in targs.pyplot:
                    # syntax: funcargs@funcname
                    eval_vargs = self.teval(opt.vargs)
                    eval_kwargs = self.teval(opt.kwargs)
                    ...
                    if len(opt.context) == 1:
                        ...
                        raise pyloco.UsageError("The synaxt error at pyplot option")

                    getattr(self.env["pyplot"], funcname)(*eval_vargs, **eval_kwargs)
